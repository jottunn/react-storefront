...
export const handleStockUpdate = async (client: Client, codStoc?: string) => {
  ...
  while (hasMoreProducts) {
    ...
    for (const product of getAllProductsArr.products) {
      ...
      for (let i = 0; i < prodVariants.length; i++) {
        ...
        const stockInfo = prodVariants[i]["stocks"];
        ...
        const stock = defaultWarehouseStock
          ...
          : prodVariants[i]["stocks"][0]?.["quantity"];
        const qlimit = prodVariants[i]["quantityLimitPerCustomer"];
        try {
          const response = await fetch(
            `${appUrl}/api/webhooks/get-expert-stock?articolCod=${sku}`,
            { method: "GET" }
          );
          ...
          if (sku && getExpertStoc !== null && getExpertStoc >= 0 && stock !== getExpertStoc) {
            console.log(
              `update stoc ${getExpertStoc} for sku ${sku} with initial stoc of ${stock}`
            );
          ...
          messages.push(
            `Error fetching or updating stock for SKU ${sku}: ${(error as any).message}`
          );
        }
      }
    }
  ...
  console.log(`Finished stock update ${messages}`);
  return messages;
};
