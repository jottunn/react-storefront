        ...
        {bannerAttribute?.values[0]?.name && (
          <>
            {(() => {
              const image = (
                <Image
                  ...
                  className={clsx(
                    ...
                    "w-full object-center hover:brightness-105 hover:contrast-115 transition-all duration-30 ",
                  )}
              ...
              return bannerLink && displayTextBanner === "NO" ? (
                <Link href={bannerLink} className="h-full w-full overflow-hidden">
                  {image}
                </Link>
              ) : (
                image
